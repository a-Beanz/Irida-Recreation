...
public var hudAssets:Array<FlxSprite> = [];
var index:Int = 1;
...
function onEvent(event) {
    if (event.event.name == "Cinematics") {
        ...
        if (speed > 0 && distance > 0) {
            FlxTween.tween(upperBar, {y: initialYUpperBar + distance}, speed, {ease: FlxEase.quadOut});
            FlxTween.tween(lowerBar, {y: initialYLowerBar - distance}, speed, {ease: FlxEase.quadOut});

 
        }
        if (distance <= 0) {
            FlxTween.tween(upperBar, {y: initialYUpperBar}, speed, {ease: FlxEase.quadIn});
            FlxTween.tween(lowerBar, {y: initialYLowerBar}, speed, {ease: FlxEase.quadIn});
        }
    }
}
